...
/**
  ...
  * YRT 20100510 fix bug 13/2 PLL multiplication factor
...
{
  uint32_t tmp = 0, pllmull1 = 0, pllmull2 = 1,pllsource = 0, presc = 0;
  ...
  switch (tmp)
  {
    ...
    case 0x08:  /* PLL used as system clock */
      ...
      pllmull1 = RCC->CFGR & CFGR_PLLMull_Mask;
      pllsource = RCC->CFGR & CFGR_PLLSRC_Mask;
      
#ifndef STM32F10X_CL      
      pllmull1 = ( pllmull1 >> 18) + 2;
      
      if (pllsource == 0x00)
      {/* HSI oscillator clock divided by 2 selected as PLL clock entry */
        RCC_Clocks->SYSCLK_Frequency = (HSI_VALUE >> 1) * pllmull1;
      }
      else
      {
 #if defined (STM32F10X_LD_VL) || defined (STM32F10X_MD_VL) || defined (STM32F10X_HD_VL)
       ...
       RCC_Clocks->SYSCLK_Frequency = (HSE_VALUE / prediv1factor) * pllmull1; 
 #else
        /* HSE selected as PLL clock entry */
        if ((RCC->CFGR & CFGR_PLLXTPRE_Mask) != (uint32_t)RESET)
        {/* HSE oscillator clock divided by 2 */
          RCC_Clocks->SYSCLK_Frequency = (HSE_VALUE >> 1) * pllmull1;
        }
        else
        {
          RCC_Clocks->SYSCLK_Frequency = HSE_VALUE * pllmull1;
        }
 #endif
      }
#else
      pllmull1 = pllmull1 >> 18;
      ...
      if (pllsource == 0x00)
      {/* HSI oscillator clock divided by 2 selected as PLL clock entry */
        RCC_Clocks->SYSCLK_Frequency = ((HSI_VALUE >> 1) * pllmull1 ) / pllmull2;
      }
      else
      {/* PREDIV1 selected as PLL clock entry */
        ...
        if (prediv1source == 0)
        { /* HSE oscillator clock selected as PREDIV1 clock entry */
          RCC_Clocks->SYSCLK_Frequency = ((HSE_Value / prediv1factor) * pllmull1) / pllmull2;          
        }
        else
        {/* PLL2 clock selected as PREDIV1 clock entry */
          ...
          RCC_Clocks->SYSCLK_Frequency = ((((HSE_VALUE / prediv2factor) * pll2mull) / prediv1factor) * pllmull1) / pllmull2;                         
        }
      }
#endif /* STM32F10X_CL */ 
      break;

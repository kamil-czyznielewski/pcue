...
void SPI_Config( void )
{
    ...
    RCC_PERIPH_GPIO_CLOCK_CMD(SD_SPI_GPIO_PORT_CLOCK | SD_CS_GPIO_PORT_CLOCK, ENABLE);

#ifdef STM32F10X_CL

    /* MSD_SPI_PORT and MSD_CS_PORT Periph clock enable */
    RCC_PERIPH_GPIO_CLOCK_CMD( RCC_APB2Periph_AFIO, ENABLE );

    /* MSD_SPI Periph clock enable */
    RCC_APB1PeriphClockCmd( SD_SPI_CLOCK, ENABLE );

    /* Remap the SPI pins if needed */
    GPIO_PinRemapConfig( SD_SPI_GPIO_REMAP, ENABLE );

    /* Configure MSD_SPI pins: SCK, MISO and MOSI */
    GPIO_InitStructure.GPIO_Pin = SD_SPI_PIN_SCK | SD_SPI_PIN_MISO | SD_SPI_PIN_MOSI;
    ...
    GPIO_PinAFConfig((GPIO_TypeDef*)SD_SPI_PORT, SD_SPI_SCK_SOURCE, SD_SPI_SCK_AF);

    /* Connect PXx to SD_SPI_MISO */
    GPIO_PinAFConfig((GPIO_TypeDef*)SD_SPI_PORT, SD_SPI_MISO_SOURCE, SD_SPI_MISO_AF); 

    /* Connect PXx to SD_SPI_MOSI */
    GPIO_PinAFConfig((GPIO_TypeDef*)SD_SPI_PORT, SD_SPI_MOSI_SOURCE, SD_SPI_MOSI_AF);  

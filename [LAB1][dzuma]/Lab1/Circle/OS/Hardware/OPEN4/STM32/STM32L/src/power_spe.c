...
void POWER_Init( void )
{
    ...
    /* Configure LP_Mode as push-pull output  */
    RCC_PERIPH_GPIO_CLOCK_CMD( GPIO_PWR_PERIPH_LP, ENABLE );
    GPIO_InitStructure.GPIO_Pin   = GPIO_PWR_LP_MODE_PIN;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
    GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_40MHz;
    GPIO_Init( GPIOx_PWR_LP, &GPIO_InitStructure );

    /* Set Low Power mode off */
    GPIO_WriteBit( GPIOx_PWR_LP, GPIO_PWR_LP_MODE_PIN, Bit_SET );

...
* @author   EL
* @date     07/2009
* @Note     split from menu.c and menu_settings.c
**/
/******************************************************************************/
...
enum MENU_code FctApp( void );
enum MENU_code FctAppIni( void );
enum MENU_code FctAppMgr( void );
enum MENU_code LaunchAppli( index_t index );

/* Private variables ---------------------------------------------------------*/
u8      TextBuffer[ 10 ];
s8      RecordPlaySeq = -1;
bool    AUDIO_BuzzerOn = 1;             // Buzzer is active or not
bool    AutorunOn = 0;                  // Application autorun or not
bool    MENU_BigFont = 0;               // Menu with char magni = 2 or not (not implemented for this Primer)

/* Extern variables ---------------------------------------------------------------*/
#ifdef PRIMER2
extern tListMin ListMenu;
extern tMenu InputMenu;
extern tMenu InterfaceMenu;
extern tMenu PowerMenu;
#endif
...
extern CONST_DATA u8* Test_Melody;
...
#ifdef PRIMER2 //FL090123 to reduce the overall power consumption.
const uint_t CCR_BackLight_Tab[ 5 ]  = {0x4000, 0x6000, 0x8000, 0xA000, 0xC000};
#endif
#ifdef PRIMER1
const uint_t CCR_BackLight_Tab[ 5 ]  = {0x1000, 0x4000, 0x8000, 0xC000, 0xFFFF};
#endif
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fAboutIni( void )
{
    FirstDisplay = 1;
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fAboutMgr( void )
{
...
#ifdef PRIMER2
        freeMemoryFlash               = 512 - 144 - AppSizeCounter;
        DRAW_DisplayString( 20,   110,  "STM32Primer2",        12 );
        DRAW_DisplayString( 0,    84,  "CPU: STM32F103VET6",  18 );
#endif
#ifdef PRIMER1
        freeMemoryFlash               = 128 - 40 - AppSizeCounter;
        DRAW_DisplayString( 20,   110,  "STM32Primer",         11 );
        DRAW_DisplayString( 0,    84,  "CPU: STM32F103RBT6",  18 );
#endif
        DRAW_DisplayString( 2,    62,  "OS : V3.3    24KB",   17 );
        DRAW_DisplayString( 2,    48,  "App: 2       16KB",   17 );
        DRAW_DisplayString( 2,    34,  "Avail FLASH: 88KB",   17 );

        DRAW_SetTextColor( RGB_BLUE );
        DRAW_DisplayString( 37,    62,  UTIL_GetVersion(),      6 );
#ifdef PRIMER2
        DRAW_DisplayString( 86,    62,  "144",                  3 );
#endif
#ifdef PRIMER1
        DRAW_DisplayString( 93,    62,  "40",                   2 );
#endif
        UTIL_uint2str( TextBuffer, LastApplication + 1, 2, 0 );
        DRAW_DisplayString( 37,    48,  TextBuffer,             2 );

        UTIL_uint2str( TextBuffer, AppSizeCounter, 3, 0 );
        DRAW_DisplayString( 86,    48,  TextBuffer,             3 );

        UTIL_uint2str( TextBuffer, freeMemoryFlash , 3, 0 );
        DRAW_DisplayString( 86,    34,  TextBuffer,             3 );

        DRAW_SetTextColor( RGB_MAKE( 0, 183, 239 ) );
        DRAW_SetBGndColor( RGB_BLACK );
        LCD_FillRect_Circle( 0, 0, Screen_Width, 20, RGB_BLACK );
        DRAW_DisplayString( 8,      2,  "STM32circle.com",      16 );
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fInMems( void )
{
    JoystickAsInput = 0;
    MemsAsInput = 1;
    TchscrAsInput = 0;
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fInJoystick( void )
{
    JoystickAsInput = 1;
    MemsAsInput = 0;
    TchscrAsInput = 0;
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fInBoth( void )
{
    JoystickAsInput = 1;
    MemsAsInput = 1;
    TchscrAsInput = 0;
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fInTchscr( void )
{
    JoystickAsInput = 0;
    MemsAsInput = 0;
    TchscrAsInput = 1;
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fCalibration( void )
{
#if TOUCHSCREEN_AVAIL
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fTest_Mgr( void )
{
    int fRedrawn = 0;
    ...
    if ( fFirstTime )
    {
        ...
        AUDIO_SpeakerOn = 1;
        UTIL_SetPll( SPEED_VERY_HIGH ); // Added after oscillator bug
        fFirstTime = 0;
    }
#endif
    ...
    if ( FirstDisplay )
    {
        ...
        DRAW_DisplayString( 0, Screen_Height - Char_Height, "  Hardware Test    ", 22 );
        DRAW_SetBGndColor( RGB_WHITE );
        DRAW_SetTextColor( RGB_BLACK );

        // Drawn that rectangle on the screen.
        LCD_FillRect_Circle( LEFT_MARGIN + 1, BAR_BOTTOM + 1, MAXLEVEL * WIDTH_SQUARE  , WIDTH_SQUARE - 2, RGB_YELLOW );
        LCD_DrawRect( LEFT_MARGIN, BAR_BOTTOM, MAXLEVEL * WIDTH_SQUARE + 2 , WIDTH_SQUARE, RGB_BLUE );

        // Restrict the move of the pointer to a rectangle.
        POINTER_SetRect( LEFT_MARGIN + 1, BAR_BOTTOM + 1, MAXLEVEL * WIDTH_SQUARE - POINTER_WIDTH - 2, WIDTH_SQUARE - POINTER_WIDTH - 2 );
    ...
    if (( AUDIO_Playback_status == NO_SOUND ) && ( AUDIO_Recording_status == NO_RECORD ) )
    {
        ...
        if (( RecordPlaySeq == 0 ) || ( RecordPlaySeq == -1 ) )
        {
            ...
            if ( RecordPlaySeq != -1 )
            {
                ...
                DRAW_DisplayString( LEFT_MARGIN, 80, "** RECORDING **", 15 );
                RecordPlaySeq = 1;
            }
            else
                RecordPlaySeq = 2;
...
//YRT090910 ??     AUDIO_SetMode(AUDIO_CIRCULAR_MODE, LG_16_BITS, FRQ_16KHZ, MONO);
            ...
            if ( RecordPlaySeq != 2 )
            {
                ...
                LED_Set( LED_RED, LED_OFF );
                DRAW_DisplayString( LEFT_MARGIN, 80, " ** PLAYING ** ", 15 );
            }
            RecordPlaySeq = 0;
        }
    }
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fSetInput( void )
{
    MENU_Set( &InputMenu );
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fSetSpeaker( void )
{
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fSetBeep( void )
{
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fInterface( void )
{
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fPower( void )
{
    MENU_Set( &PowerMenu );
...
**/
/******************************************************************************/
NODEBUG enum MENU_code fSetAutorun( void )
{
...
**/
/******************************************************************************/
NODEBUG void MENU_SetCurrentApp( void )
{
    tMenuItem*     curapp = 0;
    long unsigned  addr;

...
int getSCLK0(unsigned int _CLKIN)
{			
...
int getCCLK0(unsigned int _CLKIN)
{			
	unsigned short tempPLLCTL, tempPLLDIV;	
...
void UART0_initialize(int rate)
{	
...
{
	UART0_waitForTransferCompletion();
	*pUART0_GCTL = 0;	
}

// ****************************************************************************************************

void UART0_waitForTransferCompletion(void)
{	
...
void UART0_write(char *c, int size)
{	
	for ( ; size>0 ; --size,++c) UART0_putc(*c);			
}

// ****************************************************************************************************

char UART0_checkc(void)
{
	return (*pUART0_LSR & DR) ? 1 : 0;
}
...
int UART0_gets(char *str, int max)
{
	int i;
	for(i = 0; i < max; i++) {
		str[i] = UART0_getc();

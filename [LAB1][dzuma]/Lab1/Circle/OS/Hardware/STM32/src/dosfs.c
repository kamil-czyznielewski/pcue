...
NODEBUG uint32_t DFS_GetFAT( PVOLINFO volinfo, uint8_t* scratch, uint32_t* scratchcache, uint32_t cluster )
{
    ...
    sector = DFS_ldiv( offset, SECTOR_SIZE ).quot + volinfo->fat1;
    ...
    offset = DFS_ldiv( offset, SECTOR_SIZE ).rem;
    ...
    *successcount = 0;
    ...
    while ( remain && result == DFS_OK )
    {
        ...
        sector = fileinfo->volinfo->dataarea +
                 ...
                 DFS_div( DFS_div( fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE ).rem, SECTOR_SIZE ).quot;
        ...
        if ( DFS_div( fileinfo->pointer, SECTOR_SIZE ).rem )
        {
            ...
            /* just read.*/
            tempreadsize = SECTOR_SIZE - ( DFS_div( fileinfo->pointer, SECTOR_SIZE ).rem );
        ...
        *successcount += bytesread;
        ...
        if ( DFS_div( fileinfo->pointer - bytesread, fileinfo->volinfo->secperclus * SECTOR_SIZE ).quot !=
                DFS_div( fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE ).quot )
        {
...
NODEBUG void DFS_Seek( PFILEINFO fileinfo, uint32_t offset, uint8_t* scratch )
{
    ...
    if ( DFS_div( fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE ).quot ==
    ...
    else
    {
        ...
        fileinfo->pointer = DFS_div( fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE ).quot *
    ...
    *successcount = 0;
    ...
    while ( remain && result == DFS_OK )
    {
        ...
        if ( DFS_div( fileinfo->pointer, SECTOR_SIZE ).rem )
        {
            ...
            tempsize = DFS_div( fileinfo->pointer, SECTOR_SIZE ).rem;
            ...
            {
                DFS_memcpy( scratch + tempsize, buffer, SECTOR_SIZE - tempsize );
                if ( !result )
            ...
            {
                DFS_memcpy( scratch + tempsize, buffer, remain );
                if ( !result )
                    result = DFS_WriteSector( fileinfo->volinfo->unit, scratch, sector, 1 );
        ...
        *successcount += byteswritten;
        ...
        /* check to see if we stepped over a cluster boundary*/
        if ( DFS_div( fileinfo->pointer - byteswritten, fileinfo->volinfo->secperclus * SECTOR_SIZE ).quot !=
                DFS_div( fileinfo->pointer, fileinfo->volinfo->secperclus * SECTOR_SIZE ).quot )
        {

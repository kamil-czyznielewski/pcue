...
const s32 OrientationOffsetX [] = { +0   /* V12*/,  0    /* V3*/,    0/* V6*/,  0   /* V9*/ , +0 /*V12 at reset */};
const s32 OrientationOffsetY [] = { 0    /* V12*/,  0    /* V3*/,    0/* V6*/,  0   /* V9*/ , +0 /*V12 at reset */};
u16 LCDStatus;
...
void LCD_Interface_Init( void )
{
    ...
    RCC_PERIPH_GPIO_CLOCK_CMD( GPIO_LCD_D_PERIPH, ENABLE );
    ...
    /* RS : configured as regular GPIO  */
    RCC_PERIPH_GPIO_CLOCK_CMD( GPIO_LCD_RS_PERIPH, ENABLE );
    GPIO_InitStructure.GPIO_Pin   = CtrlPin_RS;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
    GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_40MHz;
    GPIO_Init( GPIOx_RS_LCD, &GPIO_InitStructure );

    /* RD : configured as regular GPIO  */
    RCC_PERIPH_GPIO_CLOCK_CMD( GPIO_LCD_RD_PERIPH, ENABLE );
    GPIO_InitStructure.GPIO_Pin   = CtrlPin_RD;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
    GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_40MHz;
    GPIO_Init( GPIOx_RD_LCD, &GPIO_InitStructure );

    /* WR : configured as regular GPIO  */
    RCC_PERIPH_GPIO_CLOCK_CMD( GPIO_LCD_WR_PERIPH, ENABLE );
    GPIO_InitStructure.GPIO_Pin   = CtrlPin_WR;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
    GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_40MHz;
    GPIO_Init( GPIOx_WR_LCD, &GPIO_InitStructure );

    /* Reset : configured as regular GPIO */
    RCC_PERIPH_GPIO_CLOCK_CMD( GPIO_LCD_RST_PERIPH, ENABLE );
    ...
    RCC_PERIPH_GPIO_CLOCK_CMD( GPIO_LCD_CS_PERIPH, ENABLE );
    ...
    GPIO_WriteBit( GPIOx_RD_LCD, CtrlPin_RD, Bit_SET );     /* RD = 1 */
    GPIO_WriteBit( GPIOx_WR_LCD, CtrlPin_WR, Bit_SET );     /* WR = 1 */
   ...
   void LCD_BackLightConfig( void )
{
    ...
    /* Enable GPIO clock  */
    RCC_PERIPH_GPIO_CLOCK_CMD( GPIO_LCD_BL_PERIPH, ENABLE );
    ...
    /* Connect PXx to TIM*/
    GPIO_PinAFConfig(GPIOx_BL_LCD, GPIO_BL_SOURCE_PIN, GPIOx_BL_LCD_AF);
...
{
    // Write only bits 0..7
    GPIOx->ODR = (( GPIOx->ODR ) & ~LCD_DATA_PINS ) | (( u8 )PortVal );
}
...
//    LCD_SendLCDCmd(regadd);
    ...
    GPIOx_RS_LCD->BSRRH = CtrlPin_RS;         /* RS = 0 */

    // Write datas (only bits 0..7)
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS );
    GPIOx_WR_LCD->BSRRH = CtrlPin_WR;         /* WR = 0 */
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */

    /* Start the LCD send data sequence for the second byte */

    // Write data (only bits 0..7)
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | (( u8 )regadd );
    GPIOx_WR_LCD->BSRRH = CtrlPin_WR;         /* WR = 0 */
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */
...
//    LCD_SendLCDData(value >> 8);
    ...
    GPIOx_RS_LCD->BSRRL = CtrlPin_RS;         /* RS = 1 */
    GPIOx_WR_LCD->BSRRH = CtrlPin_WR;         /* WR = 0 */

    /* Write data to the LCD */
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | (( u8 )( value >> 8 ) );
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */
...
//    LCD_SendLCDData(value & 0xFF);
    ...
    /* Write data to the LCD */
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | (( u8 )( value & 0xFF ) );
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */
}
...
void LCD_SetLogoBW( void )
{
    ...
    TextColor = RGB_BLUE;
    BGndColor = RGB_WHITE;
    DRAW_DisplayStringWithMode( 0, 26, "Welcome to STM32L Primer", ALL_SCREEN, NORMAL_TEXT, CENTER );
    TextColor = RGB_BLACK;
}
...
//    LCD_SendLCDCmd(ILI9325_GRAM_DATA);
    ...
    // Write data (only bits 0..7)
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | (( u8 )ILI9325_GRAM_DATA );
    GPIOx_WR_LCD->BSRRH = CtrlPin_WR;         /* WR = 0 */
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */
}
...
//    LCD_SendLCDCmd(ILI9325_ENTRY_MOD);
    ...
    // Write data (only bits 0..7)
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | (( u8 )ILI9325_ENTRY_MOD );
    GPIOx_WR_LCD->BSRRH = CtrlPin_WR;         /* WR = 0 */
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */
}
...
//    LCD_SendLCDCmd(ILI9325_GRAM_DATA);
    ...
    /* Start the LCD send data sequence for the second byte */
    // Write data (only bits 0..7)
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | ( ILI9325_GRAM_DATA );
    GPIOx_WR_LCD->BSRRH = CtrlPin_WR;         /* WR = 0 */
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */
...
//    LCD_DataLinesConfig(Input);
    ...
    GPIOx_RD_LCD->BSRRH = CtrlPin_RD;        /* RD = 0 */
    temp1 = ( u8 )(( GPIOx_D_LCD->IDR & LCD_DATA_PINS ) );
    GPIOx_RD_LCD->BSRRL = CtrlPin_RD;        /* RD = 1 */
    ...
    // Read data (only bits 0..7)
    GPIOx_RD_LCD->BSRRH = CtrlPin_RD;        /* RD = 0 */
    delay( 1 );                              // if no delay reading pb !
    temp1 = ( u8 )((( GPIOx_D_LCD->IDR & LCD_DATA_PINS ) ) & 0xFF );
    GPIOx_RD_LCD->BSRRL = CtrlPin_RD;        /* RD = 1 */

    // Read data (only bits 0..7)
    GPIOx_RD_LCD->BSRRH = CtrlPin_RD;        /* RD = 0 */
    delay( 1 );                              // if no delay reading pb !
    temp2 = ( u8 )((( GPIOx_D_LCD->IDR & LCD_DATA_PINS ) ) & 0xFF );
    GPIOx_RD_LCD->BSRRL = CtrlPin_RD;        /* RD = 1 */
...
void LCD_SendLCDData16( u8 Data_h, u8 Data_l )
{
    ...
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | (( u8 )Data_h );
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */

    GPIOx_WR_LCD->BSRRH = CtrlPin_WR;         /* WR = 0 */

    /* Write second data to the LCD */
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | (( u8 )Data_l );
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */
}
...
void LCD_SendLCDCmd( u8 Cmd )
{
    ...
    // Write data (only bits 0..7)
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | (( u8 )Cmd );
    GPIOx_WR_LCD->BSRRH = CtrlPin_WR;         /* WR = 0 */
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;         /* WR = 1 */
}
...
void LCD_SendLCDData( u8 Data )
{
    ...
    /* Write data to the LCD */
    GPIOx_D_LCD->ODR = (( GPIOx_D_LCD->ODR ) & ~LCD_DATA_PINS ) | (( u8 )Data );
    GPIOx_WR_LCD->BSRRL = CtrlPin_WR;             /* WR = 1 */
}
...
//    LCD_DataLinesConfig(Input);
    ...
    // Read data (only bits 0..7)
    LCDData = ( u8 )((( GPIOx_D_LCD->IDR & LCD_DATA_PINS ) ) & 0x00FF );
    GPIOx_RD_LCD->BSRRL = CtrlPin_RD;         /* RD = 1 */

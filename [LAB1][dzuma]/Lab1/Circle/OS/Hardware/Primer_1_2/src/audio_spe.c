...
*
* @file     audio_spe.c
* @brief    STw5094a audio codec init and management. Only in Primer 2
* @author   RemiC / YRT / FL
* @date     10/2008
* @version  4.0
...
u32 Audio_buffer_local_size = LOCAL_BUFFER_SIZE;
...
**/
/******************************************************************************/
NODEBUG void AUDIO_DeviceSoftwareReset( void )
{
    u8 reset_value = 0x2;
...
**/
/********************************************************************************/
NODEBUG void AUDIO_Init_I2C( void )
{
    ...
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_OD; /*FL080825 GPIO_Mode_AF_PP*/
...
*   - enable STM32 I2S communication to send audio samples (SPI3/I2S3 port) in DMA mode
*
...
**/
/******************************************************************************/
NODEBUG void AUDIO_Init_audio_mode( AUDIO_DeviceMode_enum mode, AUDIO_Length_enum length, AUDIO_Frequency_enum frequency, AUDIO_Format_enum format )
{
//    s32 i = 0;
    ...
    /* Enable the DMA2 Channel 2 Interrupt */
    ...
    /* GPIO configuration for I2S3 port*/
    /* /!\  ATTENTION  /!\  JTAG is disabled here, and replaced with SWD*/
...
//    UTIL_SetPll( UTIL_GetPll() );     // YRT091209 Prescaler calculated by the I2S_Init function
...
**/
/******************************************************************************/
NODEBUG void AUDIO_Init_voice_mode( AUDIO_DeviceMode_enum mode )
{
//    s32 i = 0;
    GPIO_InitTypeDef GPIO_InitStructure;
    I2S_InitTypeDef I2S_InitStructure;
...
**/
/******************************************************************************/
NODEBUG void AUDIO_Shutdown( void )
{
    /* Stop IT SPI*/
    I2S_Cmd( SPI3, DISABLE );
...
/**
*
*   Play the Welcome message to STM32 Primer 2.
*
**/
/********************************************************************************/
...
**/
/******************************************************************************/
NODEBUG void AUDIO_I2C_WriteMultByte( u8 WriteAddr, u8 NumByteToWrite, u8* pBuffer )
{
    s32 i;
    vu32 dr_temp ;
    u32 TimeOut;
...
**/
/******************************************************************************/
NODEBUG s32 AUDIO_I2C_ReadMultByte( u8 ReadAddr, u8 NumByteToRead, u8* pBuffer )
{
    s32 i;
    vu32 dr_temp ;
    u32 TimeOut;
...
**/
/******************************************************************************/
NODEBUG void AUDIO_BUZZER_SetToneFrequency( u16 freq )
{
    u16 valreg;
...
**/
/********************************************************************************/
NODEBUG void AUDIO_Cpy_Mono( void )
{
    volatile u16 i;
    s16  temp;
...
void AUDIO_Play( const sound_type* buffer, s32 size )
{
    ...
    if ( AUDIO_Format == STEREO )
    {
        ...
        DMA2_Channel2->CMAR = ( u32 )buffer;         /* Set the buffer*/
        DMA2_Channel2->CNDTR = size;                 /* Set the size*/
    }
    else
    {
        ...
        if ( AUDIO_Length == LG_16_BITS )
            size *= 2;
        Audio_buffer = buffer;                       /* Copy the pointer to real databuffer/source*/
        Audio_buffer_index = 0;                      /* Copy the pointer to real databuffer/source*/
        Audio_buffer_size = size;                    /* Size to the real databuffer/source*/
        AUDIO_Cpy_Mono();
        DMA2_Channel2->CMAR = ( u32 )Audio_buffer_local;        /* Set the buffer*/
        DMA2_Channel2->CNDTR = (( Audio_buffer_local_size ) / 2 ); /* Set the size*/
    }

    DMA_Cmd( DMA2_Channel2, ENABLE );                 /* Enable DMA Channel for Audio*/
...
void AUDIO_Playback_Stop( void )
{
    ...
    DMA_Cmd( DMA2_Channel2, DISABLE );                  /* Disable DMA Channel for Audio*/
    SPI_I2S_DMACmd( SPI3, SPI_I2S_DMAReq_Tx, DISABLE ); /* Disable I2S DMA REQ.*/

    AUDIO_Playback_status = NO_SOUND;

    /* Shutdwon codec in order to avoid non expected noices*/
